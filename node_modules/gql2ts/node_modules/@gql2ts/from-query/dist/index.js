...
const doIt = (schema, query, typeMap = {}, providedOptions = {}) => {
    ...
    const { wrapList, wrapPartial, generateSubTypeInterfaceName, printType, formatInput, generateFragmentName, generateQueryName, interfaceBuilder, typeBuilder, typeJoiner, generateInterfaceDeclaration, exportFunction, postProcessor, generateInputName, addExtensionsToInterfaceName, enumTypeBuilder, formatEnum, generateEnumName, generateDocumentation, typeMap: langTypeMap } = Object.assign({}, language_typescript_1.DEFAULT_OPTIONS, providedOptions);
    const TypeMap = Object.assign({}, language_typescript_1.DEFAULT_TYPE_MAP, langTypeMap, typeMap);
    ...
    const rootIntrospectionTypes = new Map([['__schema', '__Schema'], ['__type', '__Type']]);
    const getChildSelections = (operation, selection, parent, isUndefined = false) => {
        ...
        if (selection.kind === 'Field') {
            ...
            const originalName = selection.name.value;
            const selectionName = selection.alias ? selection.alias.value : originalName;
            let childType;
            ...
            if (originalName === '__typename') {
                ...
                else if (graphql_1.isAbstractType(parent)) {
                    const possibleTypes = parsedSchema.getPossibleTypes(parent);
                    /**
                ...
                let newParent;
                const fieldType = rootIntrospectionTypes.has(originalName) ? parsedSchema.getType(rootIntrospectionTypes.get(originalName)) : graphql_1.getNamedType(field.type);
            ...
            const selections = selection.selectionSet.selections.map(sel => getChildSelections(operation, sel, parent, false));
            const fragmentSelections = selections.filter(({ isFragment: frag }) => frag);
            const nonFragmentSelections = selections.filter(({ isFragment: frag }) => !frag);
            /**
            ...
            if (!fragmentSelections.length && anon) {
                ...
                let joinSelections = nonFragmentSelections.map(s => s.iface);
                isPartial = isUndefinedFromDirective(selection.directives);
                complexTypes.push(...flattenComplexTypes(selections));
                const interfaces = fragmentSelections.map(({ iface }) => iface);
